...
class starMarket(object):
    ...
    def starTrans(self, price, qty, direction = 'buy', orderType = 'limit'):
        info = {'price': price, 'qty': qty, 'direction': direction, 'orderType': orderType, 'account': self.account}
        ...
        buy = urllib.request.Request(url=str(base_url +
                                             '/venues/' + self.venues + '/stocks/' + self.stock + '/orders'),
...
x=starMarket('WSTTEX','PBLM','LAS67131683')
x.starTrans(50,50)
